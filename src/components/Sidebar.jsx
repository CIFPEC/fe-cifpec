      ...
      <div className="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
        <ul className="navbar-nav">
          <li className="nav-item">
            <Link
              to="/dashboard"
              className={`nav-link ${activeMenu === "dashboard" ? "active bg-gradient-dark text-white" : "text-dark"}`}
          ...
          {menuItems.filter(menu => menu.show).map((menu) => (
            <li className="nav-item" key={menu.id}>
              ...
              <ul className={`nav flex-column ms-4 submenu`} style={{ display: openMenus[menu.id] ? 'block' : 'none' }}>
