...
function Auth() {
  ...
  useEffect(() => {
    ...
    if (token && token.split(".").length === 3) {
      ...
      const userHasProfile =
        decoded?.userName || decoded?.fullName || decoded?.userPhone;
      if (userHasProfile) {
        navigate("/dashboard");
      } else {
    ...
    try {
      ...
      if (res?.errors) {
        ...
        if (res.message.toLowerCase().includes("password"))
          errorsObj.password = res.message;
        else errorsObj.email = res.message;
      } else {
        errorsObj.email = "Something went wrong. Please try again.";
      }
  ...
  const onRegister = async (e) => {
    ...
    if (!registerForm.retypePassword)
    ...
    if (
      ["Student", "Supervisor", "Coordinator"].includes(registerForm.role) &&
      !registerForm.course
    )
      ...
      setTimeout(
        () =>
          navigate("/verifyemail", {
            state: { email: registerForm.email, token: verifyToken },
          }),
        5000
      );
    } catch (error) {
  ...
  const showCourseDropdown = ["Student", "Supervisor", "Coordinator"].includes(
    registerForm.role
  );
  ...
  return (
    <div
      className="auth d-flex justify-content-center align-items-center p-3"
      style={{
    ...
    >
      <div className="form-box glass-box w-100" style={{ maxWidth: "500px" }}>
        <div className="auth-logo text-center">
          <Image
            src={Site.logo ? Site.logo : DefaultLogo}
            alt={`${Site.title} Logo`}
            className="img-fluid"
        ...
        <div className="d-flex mb-4">
          <Button
            className={`btn-switch w-50 me-2 ${tabs.login.status}`}
            onClick={showLogin}
          >
            LOGIN
          </Button>
          <Button
            className={`btn-switch w-50 ${tabs.register.status}`}
