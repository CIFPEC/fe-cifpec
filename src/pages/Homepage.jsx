...
const Homepage = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [courseRes, categoryRes, batchRes] = await Promise.all([
          axiosInstance.get("/courses?page=1&limit=100"),
          axiosInstance.get("/categories?page=1&limit=100"),
          axiosInstance.get("/batches?page=1&limit=100"),
        ]);
    ...
    const fetchProjectsByBatch = async () => {
      ...
      try {
        ...
        const res = await axiosInstance.get(
          `/batches/${batchId}/projects?${queryParams}`
        );
        ...
        setSearchResults(mappedProjects);
      } catch (err) {
    ...
    <div className="container-fluid px-3 px-md-4">
      <nav className="navbar navbar-expand-lg px-0 shadow-none border-radius-xl mt-2">
        <div className="container-fluid d-flex justify-content-between align-items-center">
          <img
            src={siteSetting.logo ? `${siteSetting.logo}` : "./Cifpec-Logo.png"}
            alt="CIFPEC Logo"
            style={{ height: "40px" }}
          />
          <Link to="/login">
      ...
      <header
        ...
        style={{
          backgroundImage: siteSetting.banner
            ? `url(${siteSetting.banner})`
            : "url('/src/assets/img/bg-homepage.jpg')",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        <div className="z-1">
          <h1 className="fs-3 fs-md-1 text-white">
            {siteSetting.textHeader || "CIFPEC Project Management System"}
          </h1>
          <p className="fs-6 fw-normal text-white">
            {siteSetting.description ||
              "Find the best final year student projects from ADTEC Melaka here."}
          </p>
        </div>
      ...
      <section className="mt-4">
        <h6 className="text-center fs-2 mb-4">
          Final Year Projects - ADTEC Melaka
        </h6>
            ...
            <div className="col-12 col-sm-6 col-lg-3" key={idx}>
              <div className="card card-blog card-plain h-100">
                ...
                <div className="card-body p-3 d-flex flex-column justify-content-between">
                  <h5 className="fs-6 fs-md-5">{project.title}</h5>
                  <Button
      ...
      {selectedProject && (
        <Modal show={show} onHide={handleClose} centered>
          ...
          <Modal.Body>
            ...
            <p>
              <strong>Course:</strong> {selectedProject.course}
            </p>
            <p>
